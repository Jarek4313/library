package com.jarek4313.gateway.filter;

import com.jarek4313.gateway.config.Carousel;
import com.jarek4313.gateway.utils.JwtUtil;
        ...
        return ((exchange, chain) -> {
           ...
           if (validator.isSecure.test(exchange.getRequest())) {
               ...
               HttpCookie authCookie = exchange.getRequest()
                       .getCookies().get(HttpHeaders.AUTHORIZATION).get(0);
               ...
               try {
                    if (activeProfile.equals("test")) {
                        ...
                        if (response.getStatusCode() == HttpStatus.OK) {
                            ...
                            if (cookiesList != null) {
                                List<java.net.HttpCookie> httpCookies = java.net.HttpCookie.parse(cookiesList.get(0));
                                for (java.net.HttpCookie cookie: httpCookies) {
                   ...
                   return exchange.getResponse().writeWith(
                           Flux.just(new DefaultDataBufferFactory().wrap(message.getBytes())));
               }
           }
